...
int cdb_make_start (struct cdb_make *c, int fd)
{
  ...
  c->fd = fd ;
  c->pos = 2048 ;
  buffer_init(&c->b, (iofunc_t_ref)&fd_write, fd, c->buf, BUFFER_OUTSIZE) ;
...
static int cdb_make_addbegin (struct cdb_make *c, unsigned int keylen, unsigned int datalen)
{
  ...
  if (buffer_putalign(&c->b, buf, 8) < 0) goto err ;
  return 0 ;
err:
  cdb_make_free(c) ;
  return -1 ;
...
int cdb_make_add (struct cdb_make *c, char const *key, unsigned int keylen, char const *data, unsigned int datalen)
{
  if ((cdb_make_addbegin(c, keylen, datalen) < 0)
   || (buffer_putalign(&c->b, key, keylen) < 0)
   || (buffer_putalign(&c->b, data, datalen) < 0)
  ...
  i = n ;
  {
    ...
    for (i = 0 ; i < 256 ; ++i)
    {
      ...
      for (j = 0 ; j < len ; j++)
      {
        ...
        if (buffer_putalign(&c->b, buf, 8) == -1) goto err0 ;
        if (!posplus(c, 8)) goto err0 ;
      }
    }
  }

  if (buffer_flush(&c->b) == -1) goto err0 ;
  if (seek_begin(c->fd) == -1) goto err0 ;
  if (buffer_putflush(&c->b, final, 2048) == -1) goto err0 ;
  cdb_make_free(c) ;
  return 0 ;
